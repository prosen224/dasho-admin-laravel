...
class DashboardController extends CI_Controller
{
    ...
    public function ClassRoomViewExpress($board_status, $board_type){
            ...
            if (count($row_next_formatted_common_result_downline) < 2) {
                ...
                do {
                    ...
                    foreach ($next_board_following as $next_board_following_index => $row_fol_next_board_following) {
                        ...
                        } else if ($next_board_following[$next_board_following_index]['board_type'] == 'Express') {
                            //It was before,,
        ...
        $data['next_formatted_common_result_downline'] = $next_formatted_common_result_downline;
        ...
        if ($CurrentUserData) {
            ...
            if ($board_type == 'Traveller') {
                if ($board_status == 1) {
                    ...
                    $result = $this->QueryBuilder_model->get_info('members', '*', array('email ="' . $this->session->userdata('email') . '"'), 1);
                    $this->db->select('sponsors.*,points.total_points,coins.total_coins,members.user_name,sponsor_name.user_name sponsors_name , members.reference_user');
                    $this->db->from('sponsors');
                    $this->db->join('members', 'members.user_id = sponsors.child_id');
                    $this->db->join('members sponsor_name', 'sponsor_name.user_id = sponsors.sponsor_id', 'left');
                    $this->db->join('points', 'members.user_id = points.user_id', 'left');
                    $this->db->join('coins', 'members.user_id = coins.user_id', 'left');
                    $this->db->order_by("members.ordering", "asc");
                    $this->db->where('members.system_user', 'Yes');
                    $board_member = $this->db->get();
                    $data['board_member'] = $board_member->result_array();
                    ...
                    $this->db->select('sponsors.*,members.user_name,points.total_points,coins.total_coins,sponsor_name.user_name sponsors_name,members.MembershipNumber,members.reference_user');
                    $this->db->from('sponsors');
                    $this->db->join('members', 'members.user_id = sponsors.child_id');
                    $this->db->join('members sponsor_name', 'sponsor_name.user_id = sponsors.sponsor_id', 'left');
                    $this->db->join('points', 'members.user_id = points.user_id', 'left');
                    ...
                    $this->db->order_by("sponsors.sales_number", "desc");
                    $common_result = $this->db->get();
                    $data['common_result'] = $common_result->result_array();

                    $formatted_common_result = array();
                    foreach ($data['common_result'] as $row_common_result) {
                        $formatted_common_result[$row_common_result['child_id']] = $row_common_result;
                    }

                    $second_formatted_common_result = array();
                    foreach ($formatted_common_result as $row_formatted_common_result) {
                        $second_formatted_common_result[] = $row_formatted_common_result;
                    }
                    $data['common_result'] = $second_formatted_common_result;
                    ...
                    $this->db->order_by("sponsors.sales_number", "desc");
                    $common_result = $this->db->get();
                    $data['common_result_downline'] = $common_result->result_array();
                ...
                foreach ($next_formatted_common_result_downline as $index => $row_next_formatted_common_result_downline) {
                    if (count($row_next_formatted_common_result_downline) < 2) {
                        ...
                        do {
                            ...
                            // echo 'nnnnn';
                            // echo '<pre>';
                            // print_r($next_board_following_user_sponsor_id);

                            // if($next_board_following['child_id']==1)
                            // {
                            //     $next_board_following_user_sponsor_id=2;
                            // }
                            // else
                            // {
                            //     $next_board_following_user_sponsor_id=$next_board_following['child_id'];
                            ...
                            //echo $next_board_following_user_sponsor_id.'....'.$i.'<br/>';


                            // foreach($next_board_following as $next_board_following)
                            // {
                            ...
                            foreach ($next_board_following as $next_board_following_index => $row_fol_next_board_following) {
                                if (count($next_formatted_common_result_downline[$index]) == 2) {
                                    ...
                                    $following_child_id_in_express[] = $next_board_following[0]['child_id'];
                                }
                            }
                    ...
                    if (count($row_next_formatted_common_result_downline) < 2) {
                        ...
                        do {
                            ...
                            foreach ($next_board_following as $next_board_following_index => $row_fol_next_board_following) {
                                if (count($next_formatted_common_result_downline[$index]) == 2) {
                                    $check_next_board_following = true;
                                } else if ($next_board_following[0]['board_type'] == 'Fourth') {
                                    $next_formatted_common_result_downline[$index][$next_board_following[$next_board_following_index]['child_id']]['child_id'] = $next_board_following[$next_board_following_index]['child_id'];
                                    if (count($unchanged_next_formatted_common_result_downline[$index]) == 1) {
                                        $next_formatted_common_result_downline[$index][$next_board_following[$next_board_following_index]['child_id']]['status'] = 's';
                                        $next_formatted_common_result_downline[$index][$next_board_following[$next_board_following_index]['child_id']]['board_type'] = $next_board_following[$next_board_following_index]['board_type'];
        ...
        if ($CurrentUserData) {
            ...
            if ($board_type == 'Traveller') {
                if ($board_status == 1) {
                    ...
                    $result = $this->QueryBuilder_model->get_info('members', '*', array('email ="' . $this->session->userdata('email') . '"'), 1);
                    $this->db->select('sponsors.*,points.total_points,coins.total_coins,members.user_name,sponsor_name.user_name sponsors_name');
                    $this->db->from('sponsors');
                    $this->db->join('members', 'members.user_id = sponsors.child_id');
                    $this->db->join('members sponsor_name', 'sponsor_name.user_id = sponsors.sponsor_id', 'left');
                    $this->db->join('points', 'members.user_id = points.user_id', 'left');
                    ...
                    // $this->db->select('sponsors.*,members.user_name,sponsor_name.user_name sponsors_name');
                    // $this->db->from('sponsors');
                    // $this->db->join('members', 'members.user_id = sponsors.child_id');
                    // $this->db->join('members sponsor_name', 'sponsor_name.user_id = sponsors.sponsor_id','left');
                    // //$this->db->join('points', 'members.user_id = points.user_id','left');
                    ...
                    $this->db->select('sponsors.*,members.user_name,points.total_points,coins.total_coins,sponsor_name.user_name sponsors_name,members.MembershipNumber');
                    $this->db->from('sponsors');
                    $this->db->join('members', 'members.user_id = sponsors.child_id');
                    $this->db->join('members sponsor_name', 'sponsor_name.user_id = sponsors.sponsor_id', 'left');
                    $this->db->join('points', 'members.user_id = points.user_id', 'left');
                    ...
                    // echo '<pre>';
                    // print_r($earned_child_id);
                    // exit;

                    // $this->db->select('sponsors.*,members.user_name,points.total_points,sponsor_name.user_name sponsors_name');
                    // $this->db->from('sponsors');
                    // $this->db->join('members', 'members.user_id = sponsors.child_id');
                    // $this->db->join('members sponsor_name', 'sponsor_name.user_id = sponsors.sponsor_id','left');
                    // $this->db->join('points', 'members.user_id = points.user_id','left');
                    ...
                    // echo '<pre>';
                    // print_r($data['formatted_common_result']);
                    // exit;
                    ...
                    if (count($row_next_formatted_common_result_downline) < 2) {
                        ...
                        do {
                            ...
                            foreach ($next_board_following as $next_board_following_index => $row_fol_next_board_following) {
                                if (count($next_formatted_common_result_downline[$index]) == 2) {
                                    $check_next_board_following = true;
                                } else if ($next_board_following[0]['board_type'] == 'Express') {
                                    $next_formatted_common_result_downline[$index][$next_board_following[$next_board_following_index]['child_id']]['child_id'] = $next_board_following[$next_board_following_index]['child_id'];
                                    if (count($unchanged_next_formatted_common_result_downline[$index]) == 1) {
                                        $next_formatted_common_result_downline[$index][$next_board_following[$next_board_following_index]['child_id']]['status'] = 's';
                                        $next_formatted_common_result_downline[$index][$next_board_following[$next_board_following_index]['child_id']]['board_type'] = $next_board_following[$next_board_following_index]['board_type'];
                ...
                foreach ($next_formatted_common_result_downline as $index => $row_next_formatted_common_result_downline) {
                    ...
                    do {
                        ...
                        $next_board_following_user_sponsor_id = $next_board_following['child_id'];
                        // print_r($next_board_following["child_id"]);die;
