...
class TransactionPassSubmit extends CI_Controller {
	...
	public function VerifyTransactionPass()
	{
		 ...
		 $GetUserID = $this->QueryBuilder_model->get_info('members','*',array('user_name ="' .$this->session->userdata('UserName').'"'),1);
		...
		$data['password'] = md5($new_password);
			...
			if($UpdatePassword)
			{
		    	...
		    	redirect('profile');
			     ...
			     $error_msg =  "Password Mismatch, Try Again";
		    	$this->session->set_flashdata('error_msg', $error_msg);
		    	redirect('profile');
			}
	...
	{
     	 $old_transaction_password = $this->input->post('old_transaction_passwsord');
		 $new_transaction_password = $this->input->post('new_transaction_password');
		 $confirm_transaction_password = $this->input->post('confirm_transaction_password');
		...
		if ($new_transaction_password == $confirm_transaction_password) 
		{ 
			$data['transaction_password'] = md5($confirm_transaction_password);
		}
		
		  $encoded_transaction_password = md5($old_transaction_password);
		...
		if($transaction_password == $encoded_transaction_password)
		{
			...
			if($UpdatePassword)
			{
				$error_msg = 'Passworod has been chnaged, Successfully';
				$this->session->set_flashdata('error_msg', $error_msg);
		    	redirect('profile');
			}
			else{
			    ...
			    $error_msg = "Current Password With Old Password getting Mismatch,Try Again";
				$this->session->set_flashdata('error_msg', $error_msg);
		    	redirect('profile');
		}		
	}
