...
$(document).ready(function()
{
    ...
    $(document).ajaxSuccess(function(event,xhr,options)
    {
        if(xhr.responseJSON)
        {
            ...
            if(xhr.responseJSON.main_content)
            {
                load_template(xhr.responseJSON.main_content);
            }
...
function animate_message(message)
{
    $("#system_message").html(message);
     ...
     $("#system_message").animate({opacity: 1,right:"100px"}).animate({opacity: 1,right:"10px"}, 500).delay(200).animate({opacity: 1,right:"50px"},2000).animate({opacity: 1,right:"-1000px"}, 5000);
}
